...
export default function App() {
  ...
  // --- LOGIKA DRAG & DROP UNTUK MOUSE & TOUCH ---
  ...
  const handleTouchMove = (e) => {
    if (draggingNodeId) {
      // Sangat penting: mencegah scroll layar di HP saat node digeser
  ...
  // --- FITUR BARU: TOMBOL GESER MANUAL DI TOOLBAR ---
  const moveNodeManual = (direction) => {
    ...
    const step = 48; // Jarak geser per klik
    ...
    // Otomatis pusatkan kamera jika node mulai keluar layar
    ...
    <div className="flex flex-col w-full h-screen font-sans overflow-hidden bg-slate-100 relative">
      ...
      {!isFocusMode && (
        <div className="absolute top-6 left-1/2 -translate-x-1/2 bg-white/90 backdrop-blur-md px-2 py-2 rounded-2xl shadow-lg border border-slate-200 flex items-center gap-1.5 z-50 overflow-x-auto max-w-full">
          ...
          {/* FITUR ARAH DI DALAM TOOLBAR */}
          <div className="flex items-center gap-1 bg-slate-100 p-1 rounded-xl border border-slate-200 shadow-inner">
            <button onClick={() => moveNodeManual('up')} title="Geser Atas" className="p-1 hover:bg-white hover:shadow-sm rounded-lg text-slate-600 transition-all"><ArrowUp size={16} /></button>
            <button onClick={() => moveNodeManual('down')} title="Geser Bawah" className="p-1 hover:bg-white hover:shadow-sm rounded-lg text-slate-600 transition-all"><ArrowDown size={16} /></button>
            <button onClick={() => moveNodeManual('left')} title="Geser Kiri" className="p-1 hover:bg-white hover:shadow-sm rounded-lg text-slate-600 transition-all"><ArrowLeft size={16} /></button>
            <button onClick={() => moveNodeManual('right')} title="Geser Kanan" className="p-1 hover:bg-white hover:shadow-sm rounded-lg text-slate-600 transition-all"><ArrowRight size={16} /></button>
          </div>
